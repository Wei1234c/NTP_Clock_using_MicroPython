...
class Matrices(max_matrix.MaxMatrix):
    ...
    def __init__(self,
        ...
        self.display_digit_count = int(
            matrix_count_row * matrix_count_column * columns_per_matrix / self.width_per_digit)
    ...
    def show_time(self, year, month, day, hour, minute, second):
        # It may take more than one second to show current time.
        # skip the period between 58~59 seconds so the clock calibrate and the on_clock bell can ring.
        if (minute in (58, 59) and second > 57) or (minute == second == 0):
            return
