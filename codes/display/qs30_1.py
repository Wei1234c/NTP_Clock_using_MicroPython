...
class QS30_1(shift_register.Shift_register):
    ...
    def _set_attr(self, index = 0, digit_value = None, colon_value = None, led_color_value = None):
        ...
        self.buffer[index] = self._gen_command_from_values(
            digit_value = digit_value if digit_value is not None else digit_attr,
    ...
    def show_time(self, year, month, day, hour, minute, second):
        ...
        # It takes 6 seconds for QS30 to show current time sequentially,
        # skip the period between 53~59 seconds so the clock calibrate and the on_clock bell can ring.
        if (minute in (58, 59) and second > 52) or (minute == second == 0):
            return

        time_string = "{0:0>2}".format(hour % 12 if hour != 12 else hour) + "{0:0>2}".format(minute) + "{0:0>2}".format(
            second)
        time_string = list(time_string)
        digits_to_list = [list(time_string[i:i + self.nixie_count]) for i in
                          range(0, len(time_string), self.nixie_count)]
